...
def is_statistically_significant(
    ...
    if shapiro_1.pvalue > 0.05 and shapiro_2.pvalue > 0.05:
        _logger.info(
            "There is no basis to reject H0 that both lists are normally distributed."
        )
        ...
        if levene_test.pvalue > 0.05:
            _logger.info(
                "There is no basis to reject H0 that the variances are equal, I perform a Student's t-test."
            )
        ...
        if t_test.pvalue > 0.05:
            _logger.info(
                "There is no basis to reject H0 that the difference in distributions is not statistically significant."
            )
        else:
            _logger.info(
                "Because the p-value is less than the assumed significance level (α=0.05), the difference between the distributions is statistically significant. We can therefore reject the null hypothesis and accept the alternative that the distributions of the two samples differ statistically significantly."
            )
    else:
        _logger.info(
            "At least one list is not normally distributed, I perform the Mann-Whitney test."
        )
